...
function initGiftsPanel() {
  ...
  function deleteGift(id) {
    ...
    localStorage.setItem('gifts', JSON.stringify(next));
  ...
  function toggleGiftStatus(id) {
    ...
    if (idx >= 0) {
      const order = ['created','sent','pending','accepted','rejected','owned'];
      const current = gifts[idx].status || 'created';
      const nextStatus = order[(order.indexOf(current)+1)%order.length];
      gifts[idx].status = nextStatus;
      localStorage.setItem('gifts', JSON.stringify(gifts));
      showNotification(`状態を「${nextStatus}」に更新しました。`, 'info');
      apply();
    }
  }
  function viewGift(g) {
